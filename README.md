...
v0.5.3 (alpha) keeps a Python CLI and optional GUI, with hot geometry and raster
...
The [0.5.3 verification record](reports/releases/v0.5.3.md) documents artifact
provenance, Linux acceptance and coverage limits.

**Linux x86_64**

```sh
chmod +x VoxelMill-0.5.3-linux-x86_64.AppImage
./VoxelMill-0.5.3-linux-x86_64.AppImage --help
./VoxelMill-0.5.3-linux-x86_64.AppImage gui
```
...
**Windows x64** — unzip `VoxelMill-0.5.3-windows-x64.zip` and run `VoxelMill\VoxelMill.exe` (CLI subcommands work from that same exe). SmartScreen may warn; Open anyway.
