...
FIELDS: dict[str, Field] = {
    ...
    'support.auto_bracing': Field('bool', tier='simple', label='Enable bracing', help='Add the diagonal brace network between standing supports after routing. Off leaves unbraced pillars.'),
    'support.allow_part_to_part': Field('bool', tier='advanced', risk='caution', help='Allow primary supports to anchor on model material. Brace networks always require a continuous support-only path to the plate or generated base.'),
    'support.drop_attached_unroutable': Field('bool', help='After routing, drop contacts that will not fit if they already have material one printer layer below. Island and manual contacts are never dropped.'),
    ...
    'support.part_to_part_avoidance': Field('number', tier='advanced', risk='caution', range=(0.0, 1.0), help='0: compare routes equally by length. 1: prefer any available plate route. At 0.5 a model route must be less than half the plate route length.'),
    ...
    'support.tip_shape': Field('choice', choices=TIP_SHAPES, help='Top contact shape. Cone tapers from the tip-base diameter to the contact diameter; cylinder keeps the contact diameter.'),
    'support.break_point_diameter_mm': Field('number', help='Optional ball at the top contact for a controlled snap-off. 0 disables it. When set it must be at least the contact diameter and must fit in the tip length plus penetration.'),
    ...
    'support.model_anchor_shape': Field('choice', choices=TIP_SHAPES, help='Shape of the bottom connector between the lower model surface and the middle pillar. Cone tapers from the middle pillar radius down to the buried endpoint; cylinder keeps one diameter throughout.'),
    ...
    'support.pillar_angle_deg': Field('number', positive=True, help='Shallowest angle from horizontal an angled branch may take when the column directly below a contact is blocked. 45 gives equal sideways travel and drop.'),
    ...
    'support.small_pillar_mode': Field('choice', choices=SMALL_PILLAR_MODES, help='Where the thin pillar class applies. Middle: the straight middle segment of any pillar short enough, part-to-part or not. Model: only the connector bridging a short model-to-model gap.'),
    'support.small_pillar_shape': Field('choice', choices=TIP_SHAPES, help='Buried end shape for a thin model connector: cone tapers to a point; cylinder keeps the shaft diameter. Both use the independent upper and lower depths below.'),
    ...
    'support.brace_spacing_mm': Field('number', positive=True, tier='simple', risk='caution', unit='mm', range=(0.1, 200.0), flag='--brace-spacing-mm', help='Vertical spacing between downward brace origins, measured from each support shoulder. Default 15 mm.'),
    ...
    'support.brace_min_height_mm': Field('number', tier='advanced', unit='mm', range=(0.0, 200.0), flag='--brace-min-height-mm', help='Minimum origin height above the plate. 0 allows every shoulder-derived level.'),
    ...
    'peel.enabled': Field('bool', tier='advanced', risk='uncalibrated', help='Run the peel-risk advisory check. Uncalibrated: it screens geometry and is not fitted to measured peel forces.'),
    ...
    'repair.max_deviation_mm': Field('number', help='Largest surface deviation voxel repair may introduce, in mm, checked against the original triangles in both directions after repair.'),
    ...
    'repair.voxel_size_mm': Field('number', tier='expert', risk='caution', unit='mm', range=(0.0, 5.0), help='Voxel pitch for voxel-based repair, in mm. 0 derives it from the maximum deviation within the memory budget.'),
    ...
    'resources.memory_gib': Field('number', minimum=0.25, tier='advanced', unit='GiB', range=(0.25, 1024.0), help='Memory budget assumed available for analysis and repair, in GiB. Minimum 0.25; several voxel and grid operations size themselves against it.'),
    'resources.workers': Field('int', minimum=0, maximum=32, message='resources.workers must be an integer from 0 to 32, where 0 derives it from the machine', tier='advanced', range=(0, 32), help='Number of worker CPUs to use, from 0 to 32. 0 derives the count from the physical cores of the machine.'),
    'resources.worker_policy': Field('choice', choices=('performance', 'efficiency', 'all'), message='resources.worker_policy must be performance, efficiency, or all', tier='advanced', help='performance picks P-cores first. efficiency picks E-cores first, for background work that must not stall an interactive session. all uses every allowed CPU with no restriction.'),
    'resources.scratch_dir': Field('optional_text', message='resources.scratch_dir must be null or a nonempty path string', tier='expert', help='Directory used for temporary files during prepare and STL repair. Null uses the system default temp location.'),
    'resources.acceleration': Field('choice', choices=('auto', 'cpu', 'cuda'), message='resources.acceleration must be auto, cpu, or cuda', tier='advanced', help='auto uses CUDA only after a successful runtime and device probe and otherwise falls back to CPU. cpu is the deterministic fallback. cuda is refused outright when no usable device is found.'),
    'resources.cuda_device': Field('int', minimum=0, message='resources.cuda_device must be a nonnegative integer', tier='advanced', range=(0, 16), help='CUDA device index to probe and use when acceleration is auto or cuda. Must be a nonnegative integer.'),
