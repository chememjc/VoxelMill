...
FIELDS: dict[str, Field] = {
    ...
    'repair.support_void_policy': Field('choice', choices=SUPPORT_VOID_POLICIES, message='repair.support_void_policy must be fail, ignore, or fill', flag='--support-void-policy', help='fail blocks export on any enclosed void or drainage bottleneck. ignore drops support-class findings only; a hollow model cavity still fails. Where one file cannot attribute a finding (validate, slice, verify), ignore and fill warn on voids no larger than a support contact and on single-cell sealed chambers. fill seals enclosed shells after an exact union and re-validates; drainage necks still need tip geometry, not a volume floor.'),
