...
| `inspect_intersections(triangles, callback=None)` | `(n,3,3)` native float32 or float64 array (evaluated as stored), optional callback | `dict` | Self-intersections: count, candidate pairs, examples (list of (i,j) tuples), method ("exact double-double predicates") |
