...
recorded in `plan2.md` Tier 2.1, which is exactly why conflating them went
...
before; it is invalid for `plate`, which carries its own fixed bevel of
`min(0.25, raft_thickness_mm/4)` via `raft_from_feet`, and for `none`. There
is no separate raft-slope key: for a ~1 mm / 30° putty-knife edge use
`base_thickness_mm`/`raft_thickness_mm` ≈ 1 with `base_edge_slope_deg=30` on
an added base (`skate`, `pad`, `grid`, …), not on `plate`.
