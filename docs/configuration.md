...
routing. `brace_model_pillars` (default `false`) is the one exception: when
set, a model-anchored pillar that has its own bottom connector is treated as
grounded from the top of that connector, so it can send and receive braces
like a plate pillar. A model pillar with no separate bottom connector, and a
thin whole model-to-model pillar (`small_pillar_mode="model"`), are still
never braced regardless of this setting — admitting either would ground a
brace through the same buried contact point a tip anchors on, which the
bracing walk otherwise never crosses. With `brace_model_pillars` on,
`brace_min_height_mm` is measured above each model pillar's own foot rather
than above the plate, since that foot may already stand well above Z=0.
`brace_destination` selects grounded supports, new base feet, or both;
...
printed model material and defaults to `true`, as CHITUBOX does. When enabled,
...
`repair.support_void_policy` (`fail` / `ignore` / `fill`, default `ignore`) is
the supported-assembly answer to that class of finding. `ignore` records
support-class voids and drainage bottlenecks as warnings (under
