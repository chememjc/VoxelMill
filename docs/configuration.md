...
| `resources` | `memory_gib=32`, `workers=2`, `scratch_dir=null` (omit the key in TOML to use its default); `acceleration="auto"` (`auto` / `cpu` / `cuda`), `cuda_device=0`, `post_slice_hook=null`; layer analysis caps worker concurrency against full-panel mask/label/EDT scratch estimates |
